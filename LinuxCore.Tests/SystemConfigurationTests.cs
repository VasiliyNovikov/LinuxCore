...
public class SystemConfigurationTests
{
    ...
    {
        var pageSize = SystemConfiguration.Get(SysConfName.PageSize);
    ...
    {
        var openMax = SystemConfiguration.Get(SysConfName.OpenMax);
    ...
    {
        var nproc = SystemConfiguration.Get(SysConfName.NprocessorsOnln);
    ...
    {
        var conf = SystemConfiguration.Get(SysConfName.NprocessorsConf);
        var onln = SystemConfiguration.Get(SysConfName.NprocessorsOnln);
    ...
    {
        var clkTck = SystemConfiguration.Get(SysConfName.ClkTck);
    ...
    {
        Assert.ThrowsExactly<LinuxException>(() => SystemConfiguration.Get((SysConfName)(-1)));
    }
}